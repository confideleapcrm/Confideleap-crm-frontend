...
const InvestorTargeting: React.FC = () => {
  ...
  useEffect(() => {
    try {
      ...
      if (Array.isArray(parsed)) {
        ...
        pref = {
          customerIds: Array.isArray(parsed.customerIds)
            ? parsed.customerIds.map(String)
            : [],
    ...
    for (const invId of investorIds) {
      try {
        ...
        companyIds.forEach((cid) => {
          ...
          const status = latest.meeting_status || latest.status || "scheduled";
      ...
      if (action === "removed") {
        setCounts((prev) => {
          ...
          if (listType === "interested") {
            ...
            next.not_interested = Math.max(0, (prev as any).not_interested - 1);
          } else if (listType === "meeting") {
        ...
        setListItems((prev) =>
          prev.filter(
            (it: any) =>
              !(it.id === item.id || it.investor_id === item.investor_id)
          )
        );
      }
    };
  ...
  const quickMarkInterestedFromFollowups = async (investorId: string) => {
    ...
    const baseItem = listItems.find((it: any) => {
      const snapId =
        it.snapshot?.investor_id || it.snapshot?.id || it.investor_id || it.id;
  ...
  const openInvestorDrawer = (investorId: string, mode?: DrawerInitialMode) => {
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div className="relative inline-flex">
                {" "}
                {/* inline-flex keeps Meeting aligned with other buttons */}
                <button
                  onClick={() => {
            ...
            <h2 className="text-lg font-semibold text-gray-900">
              {listView === "matching"
                ...
                : listView === "all"
                  ...
                  : `${
                      listView.charAt(0).toUpperCase() +
                      listView.slice(1).replace("_", " ")
                  ...
                  {displayedListItems.map((item) => {
                    ...
                    return (
                      <div key={item.id} className="relative">
                        ...
                        />
                        <button
                          onClick={() => handleDeleteSingleFromList(item.id)}
